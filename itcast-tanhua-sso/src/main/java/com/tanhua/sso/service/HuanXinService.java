...
public class HuanXinService {
    ...
    public boolean removeUsers(Long userId, Long friendId) {
        ...
        try {
            // 404 -> 对方未在环信注册
            return this.execute(targetUrl, null, HttpMethod.DELETE);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private boolean execute(String url, String body){
        return this.execute(url, body, HttpMethod.POST);
    }

    private boolean execute(String url, String body, HttpMethod httpMethod) {
        try {
            ...
            try {
                switch (httpMethod){
                    ...
                    case DELETE:{
                        responseEntity = this.restTemplate.exchange(url, HttpMethod.DELETE, httpEntity, String.class);
                        break;
                    }
                    case PUT:{
                        responseEntity = this.restTemplate.exchange(url, HttpMethod.PUT, httpEntity, String.class);
                        break;
                    }
                }
